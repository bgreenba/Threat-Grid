...
def getopts(argv):
    ...
    parser.add_argument('-a', '--after_time',
                        help='''Specify a start time for the feed window. You want the data from after this time. Must be in format "%%Y-%%m-%%dT%%H:%%M:%%SZ".
                            (default one hour ago, or one hour before before_time if specified) Only valid for non-curated/v2 feeds''',
                        type=valid_date)
    parser.add_argument('-b', '--before_time',
                        help='''Specify an end time for the feed window. You want the data from before this time. Must be in format "%%Y-%%m-%%dT%%H:%%M:%%SZ".
                            (default now, or one hour after after_time if specified) Only valid for non-curated/v2 feeds''',
                        type=valid_date)
    ...
    parser.add_argument('-d', '--feed_date', help='specify a date - for curated/v3 feeds ONLY')
    parser.add_argument('-f', '--output_format', 
                        help='specify a format - for curated/v3 feeds ONLY')
    parser.add_argument('-x', '--experiment', help='Do everything except request the feed. Most useful with -v',
                        action='store_true')
    ...
    parser.add_argument('--rtfm', help='''print a link to the API documentation for the specified feed and exit
                        ...
                        action='store_true')
    parser.add_argument('-v', '--verbose', help='print diagnostic and troubleshooting information to stdout',
                        action='store_true')
...
##if feedName is spec'd, get the type, typepath, and feedpath, otherwise exit
...
if feedType == 'curated':
    feedDate=''
    if args['feed_date'] is not None:
        feedDate='_'+args['feed_date']
else:
...
###make request string
if feedType == 'curated':
    #if this is a curated feed
    verbose('feed is curated')
    if args['output_format'] is not None:
        #if a format was spec'd use that
        outputFormat=args['output_format']    
    else:
        #if no format was specified default to json
        outputFormat='json'
    url='https://{}/{}{}{}.{}'.format(hostName, feedTypePath, feedPath, feedDate, outputFormat)
    QSitems={}
    #api_key
    QSitems['api_key'] = api_key
else:
    #this is not a curated feed and is therefore normal
    ##make query string out of parameters
    verbose('feed is not curated')
    QSitems={}
    #api_key
    QSitems['api_key'] = api_key
...
if args['experiment'] is True:  print('Request not submitted because -x or --experiment was specified on the command line')
